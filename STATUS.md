...
| sim-lab-2 | T1 (+4 subagents on case JSONs) | `src/halo/sim/` (schema+validator+cases), `src/halo/static/sim.html` (diagrams, CME, references), `tests/test_sim.py` | done | `make check` (263 passed); 5 cases (IO, OP, PMCD, canthotomy, breech), all citation-backed + per-role CME objectives (validator-enforced); IO landmark diagrams + crashing-OP death path Chrome-verified |
